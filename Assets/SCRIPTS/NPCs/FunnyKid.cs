...
public class FunnyKid : MonoBehaviour
{
    GameObject player;
    GameObject dialogue;
    ...
    void Start()
    {
        ...
        dialogue = GameObject.Find("Dialogueboy");
    ...
    void FixedUpdate()
    {
        ...
        var d1 = Vector3.Distance(GameObject.Find("PlayerArmature").transform.position, GameObject.Find("Boy1").transform.position);
        if (Input.GetKeyDown(KeyCode.E))//get e button from keyboard event 
        {
            ...
            {
                if (hit.collider.gameObject.name == "Boy1")
                {
                    if (d1 <= 3)
                    {
