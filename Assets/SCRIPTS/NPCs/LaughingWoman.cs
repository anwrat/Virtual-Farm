...
public class LaughingWoman : MonoBehaviour
{
    GameObject player;
    GameObject dialogue;
    ...
    void Start()
    {
        ...
        dialogue = GameObject.Find("Dialoguewoman2");
    ...
    void FixedUpdate()
    {
        ...
        var d1 = Vector3.Distance(GameObject.Find("PlayerArmature").transform.position, GameObject.Find("Woman2").transform.position);
        if (Input.GetKeyDown(KeyCode.E))//get e button from keyboard event 
        {
            ...
            {
                if (hit.collider.gameObject.name == "Woman2")
                {
                    if (d1 <= 3)
                    {
