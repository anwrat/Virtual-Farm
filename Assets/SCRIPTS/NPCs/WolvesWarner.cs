...
public class WolvesWarner : MonoBehaviour
{
    GameObject player;
    GameObject dialogue;
    ...
    void Start()
    {
        ...
        dialogue = GameObject.Find("Dialoguewoman4");
    ...
    void FixedUpdate()
    {
        ...
        var d1 = Vector3.Distance(GameObject.Find("PlayerArmature").transform.position, GameObject.Find("Woman4").transform.position);
        if (Input.GetKeyDown(KeyCode.E))//get e button from keyboard event 
        {
            ...
            {
                if (hit.collider.gameObject.name == "Woman4")
                {
                    if (d1 <= 3)
                    {
