...
public class TavernLady : MonoBehaviour
{
    GameObject player;
    GameObject dialogue;
    ...
    void Start()
    {
        ...
        dialogue = GameObject.Find("Dialoguewoman1");
    ...
    void FixedUpdate()
    {
        ...
        var d1 = Vector3.Distance(GameObject.Find("PlayerArmature").transform.position, GameObject.Find("Woman1").transform.position);
        if (Input.GetKeyDown(KeyCode.E))//get e button from keyboard event 
        {
            ...
            {
                if (hit.collider.gameObject.name == "Woman1")
                {
                    if (d1 <= 3)
                    {
