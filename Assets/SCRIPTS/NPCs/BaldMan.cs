...
public class BaldMan : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        dialogue = GameObject.Find("Dialogueman3");
    ...
    void FixedUpdate()
    {
        ...
        var d1 = Vector3.Distance(GameObject.Find("PlayerArmature").transform.position, GameObject.Find("Man3").transform.position);
        if (Input.GetKeyDown(KeyCode.E))//get e button from keyboard event 
        {
            ...
            {
                if (hit.collider.gameObject.name == "Man3")
                {
                    if (d1 <= 3)
                    {
